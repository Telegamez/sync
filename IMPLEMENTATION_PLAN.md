...
**Goal:** Port and clean `useSupportAgentRealtime.ts` → `useSwensyncRealtime.ts`

### Tasks

| Task ID | Task | Status | Notes |
|---------|------|--------|-------|
| 2.1 | Create types/swensync.ts with type definitions | 🔴 | Connection states, options, hook return |
| 2.2 | Create useSwensyncRealtime.ts base structure | 🔴 | Rename all references |
...
| 2.12 | Update all console logs to [Swensync] | 🔴 | Branding |
...
| Legacy Code | Swensync Code | Change Type |
|-------------|-------------|-------------|
| `SupportAgentConnectionState` | `SwensyncConnectionState` | Rename |
| `SupportAgentRealtimeOptions` | `SwensyncRealtimeOptions` | Rename |
| `SupportAgentRealtimeHook` | `SwensyncRealtimeHook` | Rename |
| `useSupportAgentRealtime` | `useSwensyncRealtime` | Rename |
| `[SupportAgent]` logs | `[Swensync]` logs | Rename |
| `search_faq` tool | ❌ Removed | Delete |
| `handleFAQSearch` | ❌ Removed | Delete |
| Telegames-specific instructions | Generic Swensync assistant | Simplify |
| `/api/support-realtime-token` | `/api/swensync-realtime-token` | Rename |

### Simplified AI Instructions Template
```
You are Swensync, a fast and friendly voice AI assistant.
...
interface SwensyncRealtimeHook {
  // ... existing fields ...
...
- [ ] All references renamed to Swensync
...
| 3.4 | Create SwensyncOverlay.tsx main component | 🔴 | Full-screen overlay |
...
| 3.9 | Update all component names to Swensync | 🔴 | Branding |
...
```
SwensyncOverlay (main container)
├── Header (top bar)
...
```typescript
interface SwensyncOverlayProps {
  isOpen: boolean;
  onClose: () => void;
...
| Task ID | Task | Status | Notes |
|---------|------|--------|-------|
| 5.1 | Create /api/swensync-realtime-token/route.ts | 🔴 | Next.js API route |
...
```typescript
// /api/swensync-realtime-token/route.ts
...
|---------|------|
| Project | Swensync |
| Hook | useSwensyncRealtime |
| Overlay | SwensyncOverlay |
| Types | SwensyncConnectionState, SwensyncAnimState |
| API | /api/swensync-realtime-token |
| Logs | [Swensync] |
