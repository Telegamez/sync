...
describe('AIPersonalityManager', () => {
  ...
  describe('setPersonality', () => {
    ...
    it('should set custom personality with instructions', () => {
      expect(manager.setPersonality(roomId, 'custom', 'Custom instructions here')).toBe(true);
      const config = manager.getConfig(roomId);
      expect(config?.personality).toBe('custom');
  ...
  describe('setCustomInstructions', () => {
    it('should update custom instructions for custom personality', () => {
      ...
      expect(manager.setCustomInstructions(roomId, 'Updated instructions')).toBe(true);
      expect(manager.getConfig(roomId)?.customInstructions).toBe('Updated instructions');
    });

    it('should reject for non-custom personality', () => {
      manager.initRoom(roomId, 'assistant');
      expect(manager.setCustomInstructions(roomId, 'New instructions')).toBe(false);
    });
    ...
    it('should reject instructions exceeding max length', () => {
      const customManager = new AIPersonalityManager({ maxCustomInstructionsLength: 100 });
      customManager.initRoom(roomId, 'custom', 'Initial');
      const longInstructions = 'a'.repeat(150);
      expect(customManager.setCustomInstructions(roomId, longInstructions)).toBe(false);
    });
  });
  ...
  describe('additional context', () => {
    ...
    it('should set additional context', () => {
      expect(manager.setAdditionalContext(roomId, 'Project: SwenSync')).toBe(true);
      expect(manager.getConfig(roomId)?.additionalContext).toBe('Project: SwenSync');
    });

    it('should reject context exceeding max length', () => {
      const customManager = new AIPersonalityManager({ maxAdditionalContextLength: 50 });
      customManager.initRoom(roomId);
      const longContext = 'a'.repeat(100);
      expect(customManager.setAdditionalContext(roomId, longContext)).toBe(false);
    });

    it('should return false for non-existent room', () => {
      expect(manager.setAdditionalContext('non-existent', 'Context')).toBe(false);
    });
  });
  ...
  describe('participant context', () => {
    ...
    it('should return false for non-existent room', () => {
      expect(manager.setParticipantContext('non-existent', 'Context')).toBe(false);
    });
  });
  ...
  describe('generateInstructions', () => {
    ...
    it('should include additional context', () => {
      manager.initRoom(roomId, 'assistant');
      manager.setAdditionalContext(roomId, 'This is a sprint planning meeting.');
      const instructions = manager.generateInstructions(roomId);
  ...
  describe('validation', () => {
    describe('validateCustomInstructions', () => {
      ...
      it('should reject instructions exceeding max length', () => {
        const customManager = new AIPersonalityManager({ maxCustomInstructionsLength: 100 });
        const result = customManager.validateCustomInstructions('a'.repeat(150));
        expect(result.valid).toBe(false);
    ...
    describe('validatePersonality', () => {
      ...
      it('should reject custom when not allowed', () => {
        const customManager = new AIPersonalityManager({ allowCustomPersonality: false });
  ...
  describe('callbacks', () => {
    it('should call onPersonalityChange when personality changes', () => {
      ...
      const callbackManager = new AIPersonalityManager({}, { onPersonalityChange });
      ...
      const event = onPersonalityChange.mock.calls[0][0] as PersonalityChangeEvent;
    ...
    it('should call onValidationError when validation fails', () => {
      ...
      const callbackManager = new AIPersonalityManager({}, { onValidationError });
      callbackManager.initRoom(roomId);
  ...
  describe('custom personality restrictions', () => {
    ...
    it('should allow preset personalities when custom is disabled', () => {
      const restrictedManager = new AIPersonalityManager({ allowCustomPersonality: false });
      restrictedManager.initRoom(roomId, 'facilitator');
      expect(restrictedManager.hasRoom(roomId)).toBe(true);
      expect(restrictedManager.getConfig(roomId)?.personality).toBe('facilitator');
    });
  });
