/**
 * OpenAI Realtime Provider Tests
 *
 * Tests for the OpenAI Realtime API provider adapter.
...
describe("OpenAIRealtimeProvider", () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    provider = new OpenAIRealtimeProvider(testApiKey);
  });
  ...
  describe("constructor", () => {
    ...
    it("should throw error without API key", () => {
      expect(() => new OpenAIRealtimeProvider("")).toThrow(
        "OpenAI API key is required",
      );
    });

    it("should enable debug mode", () => {
      const debugProvider = new OpenAIRealtimeProvider(testApiKey, true);
      expect(debugProvider.providerType).toBe("openai");
    });
  });

  describe("capabilities", () => {
    it("should have correct provider type", () => {
      expect(provider.capabilities.provider).toBe("openai");
    });
    ...
    it("should have correct model", () => {
      expect(provider.capabilities.model).toBe(
        "gpt-4o-realtime-preview-2024-12-17",
      );
    });

    it("should list all OpenAI voices", () => {
      expect(provider.capabilities.voices).toContain("marin");
      expect(provider.capabilities.voices).toContain("alloy");
      expect(provider.capabilities.voices).toHaveLength(9);
    });
    ...
    it("should not have built-in search", () => {
...
describe("OpenAIRealtimeProvider - IVoiceAIProvider interface compliance", () => {
  it("should implement all required interface methods", () => {
    const provider = new OpenAIRealtimeProvider("test-key");
