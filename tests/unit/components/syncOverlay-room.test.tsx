/**
 * SyncOverlayRoom Component Tests
 *
...
import {
  SyncOverlayRoom,
  type SyncOverlayRoomProps,
  type RoomParticipant,
  type RoomAISession,
} from "@/components/room/syncOverlayRoom";
...
describe("SyncOverlayRoom", () => {
  ...
  const defaultProps: SyncOverlayRoomProps = {
    isOpen: true,
    onClose: vi.fn(),
    roomId: mockRoomId,
    roomName: "Test Room",
  ...
  describe("rendering", () => {
    it("renders when isOpen is true", () => {
      render(<SyncOverlayRoom {...defaultProps} />);
      expect(screen.getByRole("dialog")).toBeInTheDocument();
    });

    it("does not render when isOpen is false", () => {
      render(<SyncOverlayRoom {...defaultProps} isOpen={false} />);
    ...
    it("has correct aria label with room name", () => {
      render(<SyncOverlayRoom {...defaultProps} />);
      expect(
        screen.getByLabelText("Test Room Voice Conversation"),
      ).toBeInTheDocument();
    });
  ...
  describe("connection states", () => {
    it("shows connecting state", () => {
      render(
        <SyncOverlayRoom {...defaultProps} connectionState="connecting" />,
      );
    ...
    it("shows reconnecting state", () => {
      render(
        <SyncOverlayRoom {...defaultProps} connectionState="reconnecting" />,
      );
    ...
    it("shows retry button on error", () => {
      render(<SyncOverlayRoom {...defaultProps} connectionState="error" />);
      expect(
  ...
  describe("participant display", () => {
    ...
    it("shows overflow indicator for many participants", () => {
      const manyParticipants: RoomParticipant[] = Array.from(
        { length: 7 },
        (_, i) => ({
          id: `peer-${i}` as PeerId,
          displayName: `User ${i}`,
  ...
  describe("AI state display", () => {
    ...
    it("shows listening state", () => {
      ...
      render(<SyncOverlayRoom {...defaultProps} aiSession={aiSession} />);
  ...
  describe("responding to speaker", () => {
    it("shows who AI is responding to", () => {
      const participants: RoomParticipant[] = [...defaultParticipants];
  ...
  describe("controls", () => {
    ...
    it("hides PTT button when showPTT is false", () => {
      render(<SyncOverlayRoom {...defaultProps} showPTT={false} />);
    ...
    it("hides settings button when onOpenSettings not provided", () => {
      render(<SyncOverlayRoom {...defaultProps} onOpenSettings={undefined} />);
    ...
    it("calls onOpenSettings when settings button clicked", () => {
      ...
      render(
        <SyncOverlayRoom {...defaultProps} onOpenSettings={onOpenSettings} />,
      );
  ...
  describe("close behavior", () => {
    ...
    it("closes on ESC key press", async () => {
      const onClose = vi.fn();
      render(<SyncOverlayRoom {...defaultProps} onClose={onClose} />);
  ...
  describe("accessibility", () => {
    ...
    it("has correct aria-pressed on mute button", () => {
      render(<SyncOverlayRoom {...defaultProps} isLocalMuted={true} />);
      expect(screen.getByLabelText("Unmute microphone")).toHaveAttribute(
        "aria-pressed",
        "true",
      );
    });
  ...
  describe("edge cases", () => {
    it("handles empty participants array", () => {
      render(<SyncOverlayRoom {...defaultProps} participants={[]} />);
      expect(screen.getByText("0")).toBeInTheDocument();
    });

    it("handles missing optional callbacks", () => {
      const minimalProps: SyncOverlayRoomProps = {
        isOpen: true,
        onClose: vi.fn(),
        roomId: mockRoomId,
        connectionState: "connected",
      ...
      expect(() => render(<SyncOverlayRoom {...minimalProps} />)).not.toThrow();
    });

    it("handles very long room name", () => {
      const longName = "A".repeat(100);
      render(<SyncOverlayRoom {...defaultProps} roomName={longName} />);
