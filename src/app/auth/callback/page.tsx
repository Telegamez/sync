      ...
      try {
        ...
        if (hash) {
          ...
          if (accessToken) {
            ...
            // Create session
            const expiresAt = new Date(Date.now() + (parseInt(expiresIn || '3600') * 1000));
            const session = {
              accessToken,
              refreshToken,
              ...
              user: {
                ...
                displayName: userData.user_metadata?.display_name || userData.user_metadata?.full_name || userData.email?.split('@')[0],
            ...
            document.cookie = `${AUTH_STORAGE_KEY}=${encodeURIComponent(JSON.stringify(session))}; path=/; expires=${expiresAt.toUTCString()}; SameSite=Lax`;
        ...
        if (code) {
          ...
          const tokenResponse = await fetch(`${config.url}/auth/v1/token?grant_type=authorization_code`, {
            method: 'POST',
            headers: {
          ...
          if (!tokenResponse.ok) {
            ...
            throw new Error(errorData.error_description || errorData.msg || 'Failed to exchange code');
          }
          ...
          const session = {
            ...
            user: {
              ...
              displayName: tokenData.user.user_metadata?.display_name ||
